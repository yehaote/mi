...
/**
 * ScheduledThreadPoolExecutor提够了一些以schedule的方式执行runnable的方法,
 * schedule()定时执行一次任务,
 * scheduleAtFixedRate()定时循环执行任务.
 */
public class GreenhouseScheduler {
	...
	class Terminate implements Runnable {
		...
		public void run() {
			...
			// Sine the schedular has been shut down:
			new Thread() {
				@Override
				public void run() {
	...
	// New feature: data collection // 一个数据集合
	...
	class CollectionData implements Runnable {
		...
		public void run() {
			...
			synchronized (GreenhouseScheduler.this) {
				...
				lastHumdity = lastHumdity + humidityDirection
						* rand.nextFloat();
				// Calendar must be cloned, otherwise all
				// DataPoint hold references to the same lastTime.
