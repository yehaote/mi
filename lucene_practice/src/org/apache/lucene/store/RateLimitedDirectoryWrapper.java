...
/**
 * 
 * A {@link org.apache.lucene.store.Directory} wrapper that allows {@link org.apache.lucene.store.IndexOutput} rate limiting using
 * {@link org.apache.lucene.store.IOContext.Context IO context} specific {@link org.apache.lucene.store.RateLimiter rate limiters}.
 * 
 *  @see #setRateLimiter(org.apache.lucene.store.RateLimiter, org.apache.lucene.store.IOContext.Context)
 * @lucene.experimental
 */
public final class RateLimitedDirectoryWrapper extends Directory {
  ...
  private volatile RateLimiter[] contextRateLimiters = new RateLimiter[Context
      .values().length];
  ...
  private RateLimiter getRateLimiter(Context context) {
    assert context != null;
   ...
   * {@link org.apache.lucene.store.IndexOutput} created with the given {@link org.apache.lucene.store.IOContext.Context}. Pass
   * <code>null</code> to have no limit.
   * 
   * <p>
   * <b>NOTE</b>: For already created {@link org.apache.lucene.store.IndexOutput} instances there is no
   * guarantee this new rate will apply to them; it will only be guaranteed to
   * apply for new created {@link org.apache.lucene.store.IndexOutput} instances.
   * <p>
   * <b>NOTE</b>: this is an optional operation and might not be respected by
   * all Directory implementations. Currently only {@link org.apache.lucene.store.FSDirectory buffered}
   * Directory implementations use rate-limiting.
   * 
   * @throws IllegalArgumentException
   *           if context is <code>null</code>
   * @throws org.apache.lucene.store.AlreadyClosedException if the {@link org.apache.lucene.store.Directory} is already closed
   * @lucene.experimental
   */
  public void setMaxWriteMBPerSec(Double mbPerSec, Context context) {
    ensureOpen();
    if (context == null) {
   ...
   * performed with the given {@link org.apache.lucene.store.IOContext.Context context}. Pass <code>null</code> to
   * have no limit.
   * 
   * <p>
   ...
   * @lucene.experimental
   */
  public Double getMaxWriteMBPerSec(Context context) {
    ensureOpen();
    if (context == null) {
      throw new IllegalArgumentException("Context must not be null");
    }
