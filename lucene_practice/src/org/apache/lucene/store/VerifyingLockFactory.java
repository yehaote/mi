...
/**
 * A {@link org.apache.lucene.store.LockFactory} that wraps another {@link
 * org.apache.lucene.store.LockFactory} and verifies that each lock obtain/release
 * is "correct" (never results in two processes holding the
 * lock at the same time).  It does this by contacting an
 * external server ({@link org.apache.lucene.store.LockVerifyServer}) to assert that
 * at most one process holds the lock at a time.  To use
 * this, you should also run {@link org.apache.lucene.store.LockVerifyServer} on the
 * host & port matching what you pass to the constructor.
 *
 * @see org.apache.lucene.store.LockVerifyServer
 * @see org.apache.lucene.store.LockStressTest
 */
...
public class VerifyingLockFactory extends LockFactory {
   ...
   * @param host host or IP where {@link org.apache.lucene.store.LockVerifyServer}
            is running
   * @param port the port {@link org.apache.lucene.store.LockVerifyServer} is
            listening on
  */
