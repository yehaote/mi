 ...
 *
 * 表现成一个byte[]数组,表示一个已经存在的数组中的一块(从offset开始, length长度)
 * 除非另有说明, Lucene使用这个类来表示在索引中被编码为UTF-8 byte的term.
 * 如果需要转换成Java的String(UTF-16), 不要使用new String(bytes, offset, length)这样的
 * 方式来进行装换(根据平台的不同很有可能会出错).
 */
public final class BytesRef implements Comparable<BytesRef>,Cloneable {
  /** An empty byte array for convenience */
  public static final byte[] EMPTY_BYTES = new byte[0];   // 一个空数组, 为了便利

  /** The contents of the BytesRef. Should never be {@code null}. */
  public byte[] bytes; // 真正存放内容的byte数组

  /** Offset of first valid byte. */
  public int offset; // offset, 第一个有效byte开始的地方

  /** Length of used bytes. */
  public int length; // 当前对象在byte数组中的长度
   ...
   * bytes should not be null
   * 使用整个bytes来存储当前的对象,
   * 从offset(0)开始, 使用byte[]的长度作为当前的length
   * */
   ...
   * 初始化一个空的大小为capacity的byte数组, 并且把offset和length都设为0
   */
